...
TEST(BoxBoardTest, DefaultOrientationsAreCorrect) {
    const box_board::BoxSettings box{{1.0, 2.0, 3.0},
                                     {std::vector{box_board::MarkerSettings{3}},
                                      {{1}},
                                      {{7}},
                                      {{5}},
                                      {{2}},
    ...
    const auto board{box_board::make_board(dict, box)};

    const std::vector<int> expected_ids{3, 1, 7, 5, 2, 9};
    EXPECT_EQ(board.getIds(), expected_ids);

    const auto &object_points{board.getObjPoints()};
    ...
    EXPECT_EQ(object_points[box_board::BoxSettings::forward_face_index],
              expected_forward);
    ...
    const std::vector<cv::Point3f> expected_backward{{0.5F, 0.5F, -1.5F},
                                                     ...
                                                     {-0.5F, -0.5F, -1.5F},
                                                     {0.5F, -0.5F, -1.5F}};
    EXPECT_EQ(object_points[box_board::BoxSettings::backward_face_index],
              expected_backward);
    ...
    const std::vector<cv::Point3f> expected_left{{0.5F, 0.5F, 0.5F},
                                                 ...
                                                 {0.5F, -0.5F, -0.5F},
                                                 {0.5F, -0.5F, 0.5F}};
    EXPECT_EQ(object_points[box_board::BoxSettings::left_face_index],
              expected_left);
    ...
    const std::vector<cv::Point3f> expected_right{{-0.5F, 0.5F, -0.5F},
                                                  ...
                                                  {-0.5F, -0.5F, 0.5F},
                                                  {-0.5F, -0.5F, -0.5F}};
    EXPECT_EQ(object_points[box_board::BoxSettings::right_face_index],
              expected_right);
    ...
    const std::vector<cv::Point3f> expected_up{{-0.5F, 1.0F, 0.5F},
                                               ...
                                               {0.5F, 1.0F, -0.5F},
                                               {0.5F, 1.0F, 0.5F}};
    EXPECT_EQ(object_points[box_board::BoxSettings::up_face_index],
              expected_up);
    ...
    const std::vector<cv::Point3f> expected_down{{-0.5F, -1.0F, -0.5F},
                                                 ...
                                                 {0.5F, -1.0F, 0.5F},
                                                 {0.5F, -1.0F, -0.5F}};
    EXPECT_EQ(object_points[box_board::BoxSettings::down_face_index],
              expected_down);
}
...
TEST(BoxBoardTest, RotationsAndOffsetsWork) {
    ...
    const box_board::BoxSettings box{
        {3.0, 2.0, 1.0},
        {
            std::vector{
                box_board::MarkerSettings{0, 0.5, -0.5, -quarter_circle, 0.5F},
...
TEST(BoxBoardTest, JSONDecodingWorks) {
    const box_board::BoxSettings settings{[]() {
        ...
        box_board::BoxSettings settings;
        box_board::from_json(json, settings);
        return settings;
    }()};
    ...
    const auto &forward{
        settings.markers[box_board::BoxSettings::forward_face_index]};
    ...
    const auto &backward{
        settings.markers[box_board::BoxSettings::backward_face_index]};
    EXPECT_TRUE(backward.empty());

    const auto &left{settings.markers[box_board::BoxSettings::left_face_index]};
    EXPECT_TRUE(left.empty());

    const auto &right{
        settings.markers[box_board::BoxSettings::right_face_index]};
    EXPECT_TRUE(right.empty());

    const auto &up{settings.markers[box_board::BoxSettings::up_face_index]};
    EXPECT_TRUE(up.empty());

    const auto &down{settings.markers[box_board::BoxSettings::down_face_index]};
